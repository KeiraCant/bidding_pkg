...
class FireDataPlanner(Node):
    ...
    def wind_direction_callback(self, msg: Float32):
        ...
        self.get_logger().info(f"🌪️ Received wind direction: {self.wind_direction:.1f}°")
    ...
    def remove_visited_tasks(self):
        ...
        for task_id in list(self.tasks_by_fire.keys()):
            ...
            if removed_count > 0:
                self.get_logger().info(f"🗑️ Removed {removed_count} visited tasks for {task_id}. Remaining: {len(self.tasks_by_fire[task_id])}")
            ...
            for i, pt in enumerate(grid):
                ...
                if main_fire and self.wind_direction is not None:
                    ...
                    if self.planner.is_downstream(bearing, downwind_direction):
                        ...
                        self.get_logger().info(f"⬇️ Detected downstream task {sub_task_id}: bearing {bearing:.1f}°, priority {base_priority:.1f} → {priority:.1f}")
    ...
    def plan_and_publish_path(self, replan_reason="unspecified"):
        ...
        gps_path, task_order = self.planner.plan_multi_waypoint_path(
    ...
    def fire_precip_callback(self, msg: String):
        try:
            ...
            if task_id:
                ...
                self.get_logger().info(f"🌧️ Updated 24h precip for {task_id}: {precip_mm:.2f} mm")
